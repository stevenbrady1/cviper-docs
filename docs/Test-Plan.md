...
|---------|------|---------|
| 0.10.0 | 2026-09-05 | v0.10.0 release (commit: 134514f). AUDIT-2026-09 follow-up: governance/live/isolation_sensitive suite partitions + nightly isolation canary; CI installs requirements.lock; PDF font-substitution disclosure; CV page-length report vs detail target; AI eval harness (goldens, gates, nightly); FSD/TSA figures corrected from measurement with new drift checks; DPIA Appendix C (EU AI Act); release hygiene automation. |
| 0.9.0 | 2026-06-03 | **New test journeys:** cross-border consent flow (CV-353) — consent modal gates AI calls + CV uploads to non-EU providers; status-enum parity (CV-370); per-user circuit-breaker isolation (CV-250); companies write-collision safety (CV-288). **E2E:** Critical Regression chromium gate unblocked (consent dismissal in `cloud-mode-cv-search`; `@mobile` specs scoped out of chromium). **Quarantine:** `cv-analysis-refresh-prompt` `test.fixme` on mobile-safari (CV-372 / #640) — WebKit request-drop under boot burst; covered on chromium, real fix tracked. Commit: 82e58ce9. |
| 0.8.0 | 2026-05-21 | **Version bump catching up Test-Plan with the rest of the doc suite** (was on 0.6.4 while BRD/FSD/TSA had advanced through 0.6.5 → 0.7.0 because `version-docs.sh` doesn't include Test-Plan in its `DOC_FILES` array — see commit message for follow-up). **New coverage since 0.6.4**: private-beta invite gate journeys (Phase E of public-rollout — invite code accepted/rejected/expired, gate bypassed for admin); System Health admin tab (Live Health + Secrets Inventory + Logs read-only); push-notification deep-link routing on iOS+Android (CV-343-2 — foreground toast + tap routing); Apple Distribution + APNs secret sync at backend deploy (CV-329 / CV-331); CV-241 tier-expiry cron now goes through HTTP shared-secret to in-VNet backend (was direct PostgreSQL through GH Actions, blocked by VNet private endpoint); 3 Japanese bank career boards added to seed (Mizuho EMEA / MUFG / SMBC EMEA — verified live ATS routing); AI routing chip now reflects the model that will actually run for the task group (LESSON-038 family). **Coverage closed for**: LESSON-091 (AUTH_ENABLED=false test env masking AuthMiddleware gating of shared-secret admin endpoints in prod). Commit: 1821b5bd. | **Cross-app header standardisation** — all top-level tabs (CV Analysis, Job Search, Applications, Settings, Admin, Companies, Career Insights, Skills, News Feed, FAQ, My Requests) now render their title/subtitle/AI-pill via the canonical `<PageHeader>` component. Smoke tests at desktop + mobile widths confirm: each tab shows the same centred-title pattern, the `subtitle` styling matches, and the `Powered by …` pill (where present) sits below the subtitle with the correct task-group routing (analysis / writing / research / quick). **Insights consistency journey** — opening Companies, Career Insights, Skills, News Feed in sequence shows the same visual rhythm; UK Regional Salary Comparison renders inside a standard `.card`; Companies' Estimates Table has filter selects in its own card-header (not a floating sibling); Market Benchmarks sit at the bottom of the page (reference data); Skills hero is dismissible on first visit with a "What's this?" restore link. **Empty env-var crash class** — historical anti-pattern `int(os.environ.get(NAME, "DEFAULT"))` migrated to `or` form across 22 sites; CI contract scans for the vulnerable form. **Drift contracts**: PageHeader registry-driven contract (22 tests covering 11 standardised tabs + 9 exempt) and env-var anti-pattern forbid-list (3 tests) — both fail CI if the bug class reappears. **Out of scope**: CV-241 tier-expiry cron is still failing in production due to Issue #479 (Azure PG firewall blocks GitHub Actions runner IPs) — needs an infrastructure fix outside code-only PR scope. Commit: 6921a085. |
| 0.6.3 | 2026-05-07 | **Job Search restructure journeys** — three-card layout (Search Criteria / Match Scoring / Job Sources) renders top-to-bottom in the right order; removing CV Skills no longer affects what's searched, only the resulting Fit scores. **Direct Employers favourites-first journey** — every favourited employer appears at the top of the grid regardless of alphabetical position; favourites-first ordering also holds when the favourites-only filter, industry filter, or experimental/broken toggles are flipped. **Industry-dropdown count parity** — selecting any industry from the dropdown shows exactly N cards where N matches the dropdown badge; flipping `showFavouritesOnly` / `showExperimentalSources` / `showBrokenBoards` updates both the dropdown count and the rendered grid in lockstep (15-permutation parity contract). **Mobile journeys** — at viewport ≤768px, StatusBar appears at the bottom above the mobile-bottom-nav (not at the top); search action buttons sit side-by-side; Search Source Results card is collapsed by default with the completion badge visible in the header; Help & Feedback FAB is dim until tapped; first viewport now shows ~150px more content above the fold. **Readiness badge journey** — the FIT-score column shows "Strong Match" / "Needs Tailoring" / "Skills Gap" instead of action-verb labels; badge no longer reads as a clickable button. **News Feed compact strip** — Industry News Feed page header is a single 60px row showing trend count, skill count, generated-at, Refresh; bottom footer "Generated …" is removed (deduplicated). **Cross-user data layer journeys** — switching between two users with different favourited Direct Employers now persists each user's selection across logout/login (no admin/replica leak); Pro user shows "Pro" in the Plan column on Admin Users tab. **Negative scenarios** — adding 5 favourite Banking employers and 1 experimental Banking employer (disabled) shows "Banking (5)" in the dropdown and 5 cards rendered, never "Banking (6)". **Coverage closed for LESSON-080 / Rule #54 / Rule #55 / Rule #56 / Rule #57 / Rule #58.** Commit: e79a2baf. |
| 0.6.2 | 2026-04-30 | **Insights News Feed journey (LESSON-072)** — user with a CvAnalysis but no SearchProfile lands on the Insights tab and sees a personalised briefing (target skills/titles/experience derived from their CV) instead of the generic "0 years experience" copy. Footer nudge "Upload your CV in the CV Analysis tab" only appears when neither SearchProfile nor CvAnalysis exists. **Direct Employers health-check journey (LESSON-073)** — opening the Search page auto-fires health checks for any enabled company-board or job-site source whose `health_status` is unset or stale (>24h). 212 enabled boards complete in ~170s (was timing out at ~30s with sequential 17-minute scan). Cancelled mid-flight requests still persist whatever has finished via try/finally + on_result callback. **Login footer Privacy/Terms/Status journey (LESSON-076)** — clicking each footer link from the unauthenticated login page navigates to the corresponding page with a "Back to CViper" header; clicking the back link returns to the login page. Status page renders live data from `/api/health/detailed` with fallback to `/api/health`. **Negative scenarios** — `?tab=search`, `?tab=admin`, and missing `?tab=` all correctly fall through to the login screen (no public-tab bypass for authenticated-only tabs). **Coverage closed for LESSON-072 / 073 / 076 / 077.** Commit: 80fae424. |
| 0.6.1 | 2026-04-26 | **Pro tier user-facing journeys** — admin promotes a user via `PATCH /api/admin/users/{id}/tier`; sandbox-row rejection; "PRO" badge appears in TopNav; tier-aware token budget enforced (Pro 5×, Sandbox 0.5×); read-time expiry guard demotes paused Pros before the nightly job. **Stripe Phase 2 scaffolding** — checkout-session endpoint, webhook handler, nightly demotion sweep (smoke-tested, full live-billing E2E deferred). **Login redesign journeys** — landing page renders correctly at 375 / 768 / 1280 widths; Sign In modal opens, accepts credentials, closes; benefits column inside navy hero; logo sizes match design. **CV-237 Style preset journey** — Conservative / Balanced / Creative selection persists per-user, drives the tailoring prompt. **AI regression scenarios** — personal-key user changes model → next CV analysis uses the new slug, not the env-var default (LESSON-055); admin enumerations don't leak into user-facing error chains (LESSON-051); Active and "Not Connected" badges cannot coexist on the same provider card (LESSON-052); max_tokens truncation triggers a 2× retry, not a silent half-completed result (LESSON-053). **Settings journeys** — Model dropdown renders for disconnected providers; BYO-key users see quota-exempt usage pill; Remove button no longer reports phantom error (CV-234); diagnostic redaction strips PII before bundle export (CV-235). **Coverage closed for CV-234 through CV-244 + LESSON-051 / 052 / 053 / 055.** Commit: a32272f6. |
