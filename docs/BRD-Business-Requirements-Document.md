...
|---------|------|--------|---------|
| 0.10.0 (docs amendment) | 2026-09-26 | CViper Project Team | **Landing spec refresh (#1487)** — F-043 and F-052 rewritten to the shipped 2026-08 layout (hero → `#landing-ai-setup` band at position 2 → how-it-works → product proof → product family → final CTA; the API-key guide is a modal opened from the AI-setup band, not a collapsible panel). Deployment status recorded per ADR 013: this landing is reinstate-ready code, deployed nowhere during the mothball; the live cviper.ai is the static `cviper-landing` GitHub Pages site. FSD gains §8.6 (landing component inventory); the E2E journey inventory is refreshed in 04-QUALITY Part II. Documentation only — no application change, version number unchanged. |
| 0.10.0 | 2026-09-05 | CViper Project Team | v0.10.0 release (commit: 134514f). AUDIT-2026-09 follow-up: governance/live/isolation_sensitive suite partitions + nightly isolation canary; CI installs requirements.lock; PDF font-substitution disclosure; CV page-length report vs detail target; AI eval harness (goldens, gates, nightly); FSD/TSA figures corrected from measurement with new drift checks; DPIA Appendix C (EU AI Act); release hygiene automation. |
| 0.9.0 | 2026-06-03 | CViper Project Team | **Cross-border consent system (CV-353)** — GDPR Art. 44-49 region map + two consent modals (special-category + cross-border processor); `ConsentGate` gates every AI call and CV upload before data leaves for a non-EU provider; modal shows the provider's display name, not its internal id. **Push notifications (CV-348)** — unified `push_service` dispatch, FCM HTTP v1 send module (Android), daily-digest method + cron endpoint, saved-search trigger, deploy parity for `FCM_SERVICE_ACCOUNT_JSON`. **Status enums API (CV-370)** — `/api/enums` as the single source of truth for status values, with a frontend↔backend parity guard. **Per-user circuit breaker (CV-250)** — circuit breaker scoped per-user for personal-key AI calls so one user's provider outage can't trip the breaker for everyone. **Security hardening** — CV-202 admin-gate global AI-routing writes (close non-admin RBAC hole); CV-354 redact username/email PII from auth + GDPR structured logs; CV-257 surface `decrypt_failed` signal for corrupt personal keys; CV-253 personal-key provider-visibility guards. **CV-288 companies isolation** — bulk-estimate + salary-check writes wrapped in a collision-safe helper; forbid-list guard against unwrapped `repo.save_company` in routes. **App.jsx god-component de-drilling (Pilots A + B)** — `configMode` extracted to `ConfigContext`, `advancedMode`/`showAllTabs` to `UIPrefsContext` (Context-per-domain strategy, no behaviour change). **CI/infra reliability** — migration 042 SQLite FK-name fix (Docker smoke green), valid Fernet MASTER_KEY in smoke guard, Critical Regression E2E gate unblocked (consent dismissal + `@mobile` excluded from chromium), schema-drift / event-type-registry / slow-test-gating fixes, CV-241 tier-expiry cron alert correctness (LESSON-095). **Known issue:** `cv-analysis-refresh-prompt` visual baseline quarantined on mobile-safari (CV-372 / #640 — Playwright+WebKit drops in-flight requests under boot burst; app-side loader-retry fix tracked). **53 commits total since 0.8.0.** Commit: 82e58ce9. |
| 0.8.0 | 2026-05-21 | CViper Project Team | **Public-rollout Phase D legal docs landed** — `docs/SUB_PROCESSORS.md` (transitive processor index for GDPR Art. 28 DPAs), `docs/DPIA-AI-Processing.md` (AI Data Protection Impact Assessment for the 8 model providers), `docs/SOLICITOR_BRIEF_TOS_REVIEW.md` (one-shot brief for external Terms-of-Service review before public launch). All three registered in the Documentation Registry and owned by DPO persona. **Public-rollout Phase E private-beta invite gate** — feature flag + invite-code check on `/api/auth/register`; admin-only bypass; gate releases on go-live signal. **Admin System Health tab** — Live Health (component status from `/api/health/detailed`), Secrets Inventory (read-only view of Azure Key Vault secret names + last-modified, no values), Logs panel (recent structured log events). **Mobile rollout — iOS TestFlight Build 16** — full Apple credential chain in place: Team `TT8K4YT4TQ`, manual signing with Distribution profile `UMAHD524CU` + certificate `8W7UH8F2LS` + .p12 in legacy format, ExportOptions.plist locked to `signingStyle: manual`, ITSAppUsesNonExemptEncryption=NO for auto-pass export compliance, Xcode 26+ requirement met. APPLE_* + APNs env vars synced via backend deploy (CV-329 / CV-331). CapacitorHttp plugin enabled to bypass WKWebView fetch quirks. Foreground push toast + tap deep-link routing wired (CV-343-2). App icon set on both platforms. Mobile bundles point at cviper.ai/api with DNS probe before build. **CV-241 tier-expiry cron migration completed** — direct-PostgreSQL-from-runner approach (blocked by VNet private endpoint) replaced with HTTP shared-secret POST to in-VNet backend; LESSON-091 fix landed (route now `register_public()`-wrapped so AuthMiddleware doesn't pre-empt `require_cron_secret` in prod with `AUTH_ENABLED=true`); 3 new regression tests covering production middleware stack; CI ACR login wrapped in 3-attempt retry to survive transient TLS resets. Operator action pending: set `CRON_SECRET` in GH Actions + Azure Key Vault. **AI model routing chip fix** — `ActiveModelIndicator` now resolves the per-group model from `aiPriority.resolved_models` (mirrors backend `AIGateway._resolve_quality_override` at call-time) — was showing the provider's configured default while the analysis actually ran on the quality-preset light-tier model. LESSON-038 family — single source of truth via `ai_meta`, not pre-call `provider` variable. Google AI timeout bumped to accommodate `gemini-2.5-flash-lite` 95th-percentile latency. **Schema-drift recogniser hardening (LESSON-090)** — multi-line `Column(...)` declarations no longer break the column-extraction regex; idempotency-guard recogniser broadened to `dialect.name` / `IF NOT EXISTS` / `inspector.has_table()`. **3 Japanese bank career boards added to `COMPANY_BOARDS_SEED`** — Mizuho EMEA (SuccessFactors, 5 London jobs verified via live registry probe), MUFG (Workday tenant `mufgub`, 20 London jobs verified), SMBC EMEA (SuccessFactors EU, 5 London jobs verified). Skipped 5 others: MUFG Trust / Sumitomo Mitsui Trust / Norinchukin / DBJ / JBIC — no English-language ATS-backed careers presence. **Visual-regression mobile nightly stabilised** — route-storm resilience patch in `cv-analysis-refresh-prompt.png` baseline (WebKit parallel-fetch burst on `initializeApp()` was overflowing Playwright's mock interception queue, leaving 61 of 73 `/api/` requests un-mocked). **57 commits total since 0.7.0.** **Coverage closed for**: LESSON-091. Commit: 1821b5bd. |
| 0.7.0 | 2026-05-17 | CViper Project Team | **Phase 1 of iterative CV authoring (ADR-008)** — every legacy generation now lands in a persistent draft DAG with version history, fabrication metadata, drift-score, and provider attribution. Nine cards shipped (CV-318 schema + migration 040 + ADR; CV-319 REST surface `/api/cv-drafts` with Pydantic v2; CV-320 fabrication + drift check on new POST path; CV-321 adapter-write on `/apply-single` + `/update-cv` linear lineage; CV-322 `<DraftTimeline>` component in DocumentCentre; CV-323 adapter-write on `/alternative` + `/multi-generate` sibling style; CV-324 `<DraftCompare>` picker + base-CV diff; CV-325 fabrication retrofit on legacy adapter-write path with `CV_FABRICATION_STRICT` env-var gate; CV-326 7-row coverage matrix + Playwright lifecycle E2E). Adapter-write is non-blocking — failures swallowed with `event_type=draft_dual_write_failed` log line; user response never breaks. **Native authentication** — Sign in with Apple (CV-329) and Google (CV-330) endpoints verify provider id_token and mint CViper JWT; `oauth_native` module + deploy chain. **Mobile native rollout — Capacitor scaffolding** — iOS + Android wrappers (CV-327), universal links (CV-332), push-notification backend foundation with device_tokens table + register/list/delete endpoints (CV-331a), JDK 21 / Node 22 / Capacitor 8. **GDPR — `cv_text` TTL + Pattern A/B/C** (CV-328) — 7-day idle expiry on `cv_text` with profile preserved, Refresh CV prompt drives re-upload, Pattern A (full)/B (degraded with placeholder)/C (block + prompt) wired across 5 dependent features. Native JSON refresh tokens behind `X-Client-Type` header (Capacitor can't read HttpOnly cookies). Privacy policy refreshed to v1.2. **Landing FAQ** rewritten to four trust signals (privacy, pricing, no-fabrication, demo) with forbid-list content contract. Helper button → FAQ spacing tightened so initial paint fits common laptop viewports without a scrollbar. Commit: bbd57487. |
...
| F-043 | Step-by-step API key guide for novice users (FAQ + landing `ApiKeyGuideModal`, opened from the on-page AI-setup band — the collapsible panel was retired in the 2026-08 overhaul) | Post-core |
...
| F-052 | Landing page (2026-08 overhaul): hero → AI-setup band (`#landing-ai-setup`) → how-it-works → product proof → product family → final CTA; sign-in via modal. **Reinstate-ready only** — not deployed during the mothball; the live cviper.ai is the static `cviper-landing` page (ADR 013) | Post-core |
