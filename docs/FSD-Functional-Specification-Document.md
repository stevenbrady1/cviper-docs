...
|---------|------|--------|---------|
| 0.9.0 (docs amendment) | 2026-09-26 | CViper Project Team | **Landing spec refresh (#1487)** — new §8.6 documents the pre-auth landing: the shipped 2026-08 section order (hero → `#landing-ai-setup` → how-it-works → proof → family → final CTA), the component inventory under `frontend/src/components/landing/`, the LoginScreen recomposition (slice 5A), and the ADR 013 deployment status (reinstate-ready; live cviper.ai is the static `cviper-landing` site). Documentation only — no application change. |
| 0.9.0 | 2026-06-03 | CViper Project Team | **New endpoints/surfaces:** `/api/enums` (status enums single source of truth, CV-370); push-notification dispatch — FCM HTTP v1 send + daily-digest cron + saved-search trigger (CV-348). **Cross-border consent (CV-353):** region map + two consent modals + `ConsentGate` gating all AI calls and CV uploads to non-EU providers; consent modal renders provider display name. **AI routing:** per-user circuit breaker for personal-key calls (CV-250); admin-gate on global AI-routing writes (CV-202); `decrypt_failed` signal surfaced for corrupt personal keys (CV-257). **Frontend architecture:** new `ConfigContext` (`configMode`) and `UIPrefsContext` (`advancedMode`/`showAllTabs`) extracted from App.jsx (Pilots A + B). **Data integrity:** companies write paths (bulk-estimate, salary-check) wrapped in a collision-safe helper (CV-288). Commit: 82e58ce9. |
| 0.8.0 | 2026-05-21 | CViper Project Team | **New backend endpoint** `POST /api/admin/cron/run-tier-expiry` (CV-241 follow-up) — shared-secret Bearer auth via `routes/cron.py::require_cron_secret` (hmac.compare_digest against `CRON_SECRET` env var). Path wrapped in `register_public()` so AuthMiddleware doesn't pre-empt the dependency in prod (LESSON-091). Returns `{demoted: N}` on success, 503 when `CRON_SECRET` is unset (deployment misconfig surfaced loudly), 401 on missing/wrong-scheme/invalid Bearer. Workflow: `.github/workflows/tier-expiry.yml` cron 03:00 UTC daily, replaces direct-DB Python script that broke when Postgres moved behind VNet private endpoint. **New backend invite-code gate** on `/api/auth/register` — feature flag + database table for invite codes; admin-only bypass; releases on go-live signal (Phase E of public-rollout roadmap). **New admin frontend tab** `SystemHealthTab` with three panels: Live Health (consumes `/api/health/detailed`), Secrets Inventory (read-only Key Vault names + last-modified, no values), Logs panel (recent structured log events streamed from backend). New E2E spec `frontend/e2e/admin-system-health.spec.js`. **Mobile native — iOS**: `frontend/ios/App/App/Info.plist` includes `ITSAppUsesNonExemptEncryption=NO`, `CapacitorHttp` plugin enabled in `capacitor.config.json`. iOS workflow uses Xcode 26+ runner image, ExportOptions.plist `signingStyle: manual`. Backend env vars added: `APPLE_DISTRIBUTION_P12`, `APPLE_DISTRIBUTION_P12_PASSWORD`, `APPLE_APNS_KEY_ID`, `APPLE_APNS_PRIVATE_KEY`, `APPLE_API_KEY_ID`, `APPLE_API_ISSUER_ID`, `APPLE_API_PRIVATE_KEY`, `APPLE_BUNDLE_ID`, `APPLE_TEAM_ID`, `APPLE_APP_STORE_API_KEY` — all wired into `container-apps.bicep` + `container-apps-staging.bicep`. **Push notifications — CV-343-2**: foreground toast component + tap deep-link routing in iOS + Android wrappers; backend `device_tokens` table already existed from CV-331a (no new migration). **AI routing chip — frontend fix**: `frontend/src/components/ActiveModelIndicator.jsx::resolvedModel` useMemo now consults `aiPriority.resolved_models[providerId][group]` before falling back to `provider.model` — mirrors backend `AIGateway._resolve_quality_override` at call-time. New test in `ActiveModelIndicator.test.jsx`. **Backend `_APP_VERSION`**: reads from `frontend/package.json` (now 0.8.0); exposed at `GET /` and `GET /api/health`. **Schema drift recogniser** (`backend/tests/schema/test_alembic_alignment.py`): column-extraction regex now uses balanced-paren matching; idempotency-guard test accepts `dialect.name` / `IF NOT EXISTS` / `inspector.has_table()` (was `dialect.name` + `IF NOT EXISTS` only). **`COMPANY_BOARDS_SEED`**: 226 → 229 entries (Mizuho EMEA / MUFG / SMBC EMEA at lines 226-267 of `backend/seed_company_boards.py`, clustered with existing Nomura + Nomura (Instinet) entries). **CI workflow change** (`.github/workflows/ci.yml`): `Log in to ACR` step now wraps `az acr login --name cviperacr` in a 3-attempt retry loop with linear backoff (5s, 10s) to survive transient TLS resets. **New regression tests**: `backend/tests/security/test_cron_auth.py::TestCronAuthMiddlewareExemption` (3 tests with `monkeypatch.setattr("auth.AUTH_ENABLED", True)` to exercise prod middleware stack). **Stats**: backend tests +3 (CV-241 prod-middleware coverage), frontend tests +1 (ActiveModelIndicator group resolution), seed entries +3 (Japanese banks). **Coverage closed for**: LESSON-091. Commit: 1821b5bd. |
| 0.7.0 | 2026-05-17 | CViper Project Team | **New database table**: `cv_drafts` (migration 040, CV-318) — version DAG anchored to a job and an optional base CV. Columns: id, job_id (FK), parent_draft_id (FK, nullable), base_cv_id (FK, nullable), version_number, cv_text, cover_letter_text, instruction, change_summary, structured_changes, ats_score, fabrication_check (JSON), drift_from_base_score, prompt/completion token counts, ai_provider, ai_model, created_at, is_current, superseded_at. Unique constraint `uq_cv_drafts_job_version` on (job_id, version_number). **New REST surface** (CV-319): `GET /api/cv-drafts?job_id=X` (returns `{ drafts: [...] }`), `GET /api/cv-drafts/{id}`, `POST /api/cv-drafts` (create with auto-versioning via `func.max() + 1`), `POST /api/cv-drafts/{id}/promote` (flip `is_current`, demote siblings). Pydantic v2 with `model_config = ConfigDict(extra="forbid")` on `CvDraftCreate`, `extra="allow"` on response. **Adapter-write helper** (`backend/helpers/cv_drafts_adapter.py`, CV-321/323/325) wires every legacy generation into the draft DAG without breaking the user-facing response. `dual_write_draft(...)` is non-blocking — catches all exceptions, logs `event_type=draft_dual_write_failed`, returns None. Used by `/api/apply-single` + `/api/documents/{id}/update-cv` (linear lineage, auto-promote) and `/api/alternative` + `/api/multi-generate` (sibling style — `parent_draft_id` set, auto_promote=False). **CV-325 strict-mode gate**: `CV_FABRICATION_STRICT` env var (default off) — when true, `fabrication_risk == "high"` skips draft persistence and emits `event_type=draft_blocked_high_fabrication`; filesystem write of CV/cover-letter is unaffected. Verdict sentinels (`no_base_cv`, `check_raised`, `check_malformed`) carried on the row so UI can label outcomes. **New frontend components** (CV-322/324): `frontend/src/components/DraftTimeline.jsx` — vertical card list with version, provider, ATS score, drift, fabrication verdict, Promote button. Fetches `/api/cv-drafts?job_id=X` with refreshKey-driven refetch. Lazy-loads `<DraftCompare>` only on first compare click. `frontend/src/components/DraftCompare.jsx` — word-level diff picker with Left/Right dropdowns (Base CV option appears when `baseCvText` non-empty), smart defaults (Left=Base CV, Right=current), Swap button, same-selection guard, overlay-click-to-close. **New auth endpoints**: `POST /api/auth/apple` (CV-329) verifies Apple `id_token` and mints CViper JWT; `POST /api/auth/google` (CV-330) verifies Google `id_token` and mints CViper JWT. Both live in `backend/auth/oauth_native.py`. **Native refresh tokens**: refresh-token issuance gated by `X-Client-Type` header — cookie for web, JSON body for native (Capacitor cannot read HttpOnly cookies cross-origin). **Push-notification foundation** (CV-331a): new `device_tokens` table + `POST /api/devices/register`, `GET /api/devices`, `DELETE /api/devices/{id}`. APNs + FCM sender service is CV-348 (tracked separately). **Universal links** (CV-332): iOS Universal Links + Android App Links wired. **Capacitor scaffolding** (CV-327): iOS + Android wrappers, JDK 21, Node 22, Capacitor 8. **Cv-text TTL** (CV-328): 7-day idle expiry on `cv_text` with profile preserved; `Refresh CV` prompt + Pattern A (full content) / B (degraded with placeholder) / C (block + prompt) wired across 5 dependent features. **New frontend tests**: `DraftCompare.test.jsx`, `DraftTimeline.test.jsx`, `LoginScreen.landingFaqContent.contract.test.jsx`, plus `frontend/e2e/cv-drafts-lifecycle.spec.js` Playwright lifecycle E2E. **Coverage matrix**: `docs/cv-drafts-coverage-matrix.md` — 7-row matrix (Happy/Negative/Boundary/Edge/E2E/Regression/Exploratory) per CV-326. Stats: BE tests 364 → 365, FE tests 212 → 215, components 84 → 86 (+DraftTimeline, +DraftCompare). Commit: bbd57487. |
| 0.6.4 | 2026-05-07 | CViper Project Team | **NEW frontend component**: `frontend/src/components/PageHeader.jsx` — canonical title/subtitle/AI-pill block. Props: `title` (required), `subtitle` (string\|ReactNode), `group` (AI tier — analysis/writing/research/quick), `aiPriority`, `aiProviders`, `usage`, `navigateToAiSettings`, `setActiveTab`, `rightSlot`, `providerNameOverride`. 13 unit tests + `data-testid="page-header"`. **Migrated 11 tabs**: CVAnalysisTab, SearchTab, ApplicationsTab, ConfigTab, AdminTab, CompaniesTab, CareerInsightsTab, SkillsTab, NewsFeedTab, FAQTab, MyRequests. Each now imports `PageHeader` and replaces ~10-25 lines of hand-rolled JSX with one element. **Updated chip-group contracts** (`SearchTab.modelChipGroup`, `ApplicationsTab.modelChipGroup`) to be component-name-agnostic — accept either `<PageHeader group="...">` or the legacy `<ActiveModelIndicator group="...">`. **DELETED legacy components**: `AIProviderCard.jsx` + tests, `AIMultiProviderCard.jsx` + tests; removed from `frontend/src/components/index.js` barrel + App.jsx import. **Backend code migration** for the empty-env-var crash class: 22 sites across `auth.py`, `deps.py`, `jwt_utils.py`, `anomaly.py`, `helpers/malware_scan.py`, `main.py` (7 sites), `monitoring.py` (2 sites), `routes/auth.py`, `ai/token_budget.py` (6 sites). Pattern `int(os.environ.get(NAME, "DEFAULT"))` → `int(os.environ.get(NAME) or "DEFAULT")`. Same for `os.getenv` and `float(...)` casts. **Tooling**: `docs/version-docs.sh` bug-fix — `EXISTING_MAP` is now read into a shell variable BEFORE the heredoc truncates VERSION.md (was being wiped each run); App-Version resolver has node → python → grep three-tier fallback. **Skills tab persistent state**: `localStorage.cviper_skills_hero_dismissed` flag plus subtitle restore link via `TYPOGRAPHY.LINK`. **Companies tab structural change**: `marketBenchmarksBlock` JSX hoisted into a const before the return, rendered at the page bottom; SalaryComparisonCard converted to standard `.card` markup; filter selects + bulk actions merged into the Estimates Table `card-header`. **News Feed**: compact gradient strip removed; canonical centred header restored; Refresh button absolute-positioned top-right (`rightSlot` pattern); `Generated {date}` footer caption restored. **Test infra**: NEW `backend/tests/infrastructure/test_env_var_default_pattern.py` (3 tests). NEW `frontend/src/utils/page-header-consistency.contract.test.js` (22 tests — registry-driven). Stats: components 75 → 76 (+PageHeader); BE tests 342 → 343; FE tests 199 → 200. Commit: 6921a085. |
| 0.6.3 | 2026-05-07 | CViper Project Team | **New frontend modules**: `frontend/src/utils/companyBoardSort.js` exports `favouritesFirstByCompany` comparator + 6 unit tests (used by `App.jsx` `displayedBoards` useMemo so favourites appear at the top of the Direct Employers grid). `frontend/src/utils/companyBoardVisibility.js` exports `isVisibleIgnoringIndustry` / `isVisible` / `applyVisibilityFilters` / `countVisibleInIndustry` — a single source of truth shared between the App.jsx pipeline and the SearchForm.jsx industry-dropdown count, eliminating the dropdown/grid drift that produced "Gaming (4)" with 3 cards rendered. **Backend pipeline** unchanged — this release is overwhelmingly frontend UX. **Component restructure**: SearchForm.jsx Search Criteria card split into three sibling cards (Search Criteria / Match Scoring / Job Sources). `<FitScoreBadge>` `getReadiness()` labels renamed (`Apply Now`→`Strong Match`, `Tailor CV`→`Needs Tailoring`, `Upskill`→`Skills Gap`) + new `data-testid="readiness-badge"` and `readiness-badge-lg` attributes for stable test selection. NewsFeedTab.jsx hero block replaced with a 60px compact strip rendering trend/skill/generated-at chips. **Mobile breakpoint**: StatusBar.jsx switches `position: fixed; top:` → `bottom:` under `useIsMobile(768)`, anchored above `.mobile-bottom-nav` with `env(safe-area-inset-bottom)`; corresponding switch from `padding-top` to `padding-bottom` on `.main-container` via the inline `<style>` block. SearchForm action bar uses `flexDirection: isMobile ? 'row' : 'column'` with `flex: 2 1 0` (Search) vs `1 1 0` (LinkedIn) to preserve hierarchy. SearchTab.jsx wraps the Search Source Results card-body in `{!sourceCardCollapsed && (...)}` with `useEffect` syncing collapsed state to viewport. **CSS**: `theme.css` adds a `@media (max-width: 768px)` block tightening `.card` margin-bottom, `.card-header`/`.card-body` padding, and `.card-header h3` font-size. `mobile.css` lowers `.feedback-fab` opacity to 0.7 default with full opacity on hover/focus/tap. **Removed components**: `<a className="btn btn-linkedin-header">` block + `data-testid="header-linkedin"` from SearchForm.jsx (duplicate of bottom action-bar LinkedIn CTA). `useSearch.js` no longer fires the success `showMessage(\`Search complete: N jobs found\`)` toast — the in-page sticky banner now serves as the canonical signal. **Test infrastructure**: 2 new utility test files (`companyBoardSort.test.js` 6 tests, `companyBoardVisibility.test.js` 32 tests including a 15-permutation parity contract). 2 useSearch.test.js positive assertions converted to forbid-list assertions (LESSON-033 family). **Cross-user data layer**: `repo.get_config(session, name, user_id, for_update=True)` honours row-locking on user-scoped reads + `load_config()` bypasses the per-replica `_TTLCache(30)` when `user_id is not None` (LESSON-080 / Rule #55). `_user_to_dict` now exposes the full superset matched by `/api/auth/me` (CV-287 / Rule #56) — registry-driven contract test in `test_serialiser_completeness.py` AST-walks every `_*_to_dict` helper in `repositories.py`. `salary-estimate` route auto-renames slug on cross-user collision (CV-289). **Diagram pipeline**: showcase SVGs (`frontend/public/showcase/*.svg`) generated from `docs/diagrams/*.json` via `scripts/diagram_gen.py` — direct SVG edits fail CI (LESSON-081 / Rule #57). New Playwright workflow `.github/workflows/diagram-visual-regression.yml`, contract test for the cache-buster chain (LESSON-082 / Rule #58). **Manual Search Boards**: `ManualSearchBoards` component + new backend `manual_search_boards` module + `infra(hooks)` untracked-import guard. **Cross-browser E2E**: WebKit + Mobile Safari Playwright projects added (tagged-subset only). Commit: e79a2baf. |
| 0.6.2 | 2026-04-30 | CViper Project Team | **New backend functions**: `health_check_boards_async()` in `career_search/registry.py` — parallel async variant of `health_check_boards` using `asyncio.gather` + `Semaphore(10)` + `asyncio.wait_for(timeout=8s)` per board with `on_result` callback for partial-results persistence (LESSON-073 Tier 1). Sync `health_check_boards()` retained as backward-compat wrapper. **Endpoint behaviour changes**: `POST /api/company-boards/health-check` now awaits the async variant in a `try/finally` block — partial results persist via `save_config()` even on cancellation. `GET /api/news-feed/briefing` falls back to `repo.get_latest_cv_analysis()` when no `SearchProfile` exists, and skips persisting the cached briefing when both `target_titles` and `target_skills` are empty (avoids 4h-stale generic cache after CV upload — LESSON-072). **New frontend components**: `StatusPage` (`frontend/src/components/StatusPage.jsx`) — public service-status page; pulls `/api/health/detailed` with fallback to `/api/health`; uses raw `fetch()` not `authFetch()` so unauthenticated visitors don't trigger the global 401 interceptor. **Auth-gate generalisation**: `PUBLIC_TABS` allowlist in `App.jsx` (`{ privacy, terms, status }`) replaces the hardcoded `?tab=score` bypass for unauthenticated visitors; "Back to CViper" header wraps the rendered component (LESSON-076). **Frontend useEffect additions**: `SearchForm.jsx` auto-triggers job-sites and company-boards health checks on mount when at least one enabled source has unset/stale (>24h) `health_status` — single fire per mount via `useRef` guard (LESSON-073 Tier 2). **Config additions**: `'status'` added to `EXEMPT_TABS` in `frontend/src/config/tabs.js`. **Test infrastructure**: AST forbid-list contract `test_searchprofile_direct_query.py` (LESSON-072) for routes querying `SearchProfile.user_id` without a CvAnalysis fallback; source-scan parity contract `LoginScreen.publicLinkParity.test.jsx` (LESSON-076) for footer `?tab=` links vs the `PUBLIC_TABS` allowlist. **Multi-agent infra**: staged-set snapshot guard in pre-commit (LESSON-074); LESSON-number collision guard in commit-msg hook (LESSON-077). Commit: 80fae424. |
| 0.6.1 | 2026-04-26 | CViper Project Team | **New endpoints**: `PATCH /api/admin/users/{id}/tier` (admin-only Pro promotion, sandbox-rejected, audit-logged, idempotent — CV-236 Phase 1a), `POST /api/billing/create-checkout-session` (Stripe checkout scaffold — CV-239), `POST /api/billing/webhook` (Stripe lifecycle events — CV-240). **New scheduled job**: nightly Pro demotion sweep — demotes users whose `tier_expires_at` has passed (CV-241). **Response-shape additions**: `tier` and `tier_expires_at` now flow through `/api/auth/status`, `/api/auth/me`, and `/api/auth/me/profile` payloads (CV-236 Phase 1c). Per-request expiry guard so paused Pro subscriptions stop conferring features even before the nightly job runs (CV-238). **Token-budget**: daily AI token budget now scales with tier (Pro 5×, Sandbox 0.5× — CV-244). **New frontend components**: gold `PROBadge` in TopNav, "Plan" column + `SetPlanPicker` on Admin Users tab (CV-236 Phase 1b), Style preset dropdown on CV tailoring (Conservative / Balanced / Creative — CV-237), Model dropdown rendered for disconnected providers (Settings), redesigned login landing with two-column split + Sign In modal + mobile-first brand hero. **AI orchestration**: per-user `build_client_config` model resolution funnels through `_resolve_model_for(provider_id)` instead of `os.getenv`, restoring runtime model-update mechanisms for personal-key users (LESSON-055 / Rule #46); fallback loop respects user-configured providers and never silently bills the admin (LESSON-051 / Rule #44); class-level truncation closure with opt-in retry at 2× budget (LESSON-053); empty-content detection moved upstream + JSON-mode + BulletScorer soft-hide. **UI contract**: dual-source status badges require a contradiction-class contract test (LESSON-052 / Rule #45). **Auto-correction rules** #44, #45, #46. Commit: a32272f6. |
...
### 8.6 Landing Page (pre-auth)

> **Deployment status (ADR 013):** this landing is **reinstate-ready code**,
> deployed nowhere during the 2026-09 mothball. The live `cviper.ai` is the
> static `cviper-landing` GitHub Pages site (its own repo, its own visual
> check, CV-1403). The section below specifies what ships when the hosted
> product is reinstated.

Shipped layout (2026-08 overhaul, replacing the F-052 "demo first, guide,
login below" design): **hero → AI-setup band (`#landing-ai-setup`, position
2) → how-it-works → product proof → product family → final CTA band**, with
sign-in via a modal from the header or hero rather than an inline form.

Components (`frontend/src/components/landing/`, each with a co-located test;
plus the `landingStyles.contract.test.js` style contract):

| Component | Role |
|---|---|
| `LandingPage.jsx` | Section assembly, hero, sign-in modal wiring, parallax refs |
| `LandingHeader.jsx` | Pre-auth top bar with the Sign in trigger |
| `AiSetupHelpSection.jsx` | The `#landing-ai-setup` band; opens the key guide |
| `ApiKeyGuideModal.jsx` | Step-by-step API-key guide (F-043's current home) |
| `HowItWorksSection.jsx` | Three-step usage band |
| `BrowserFrame.jsx` / `RewriteSchematic.jsx` | Product-proof figures (framed screenshots + rewrite schematic) |
| `ProductFamilySection.jsx` | Hosted + Light product-family band |
| `FinalCtaBand.jsx` | Closing call-to-action |
| `WaitlistModal.jsx` | Waitlist capture (its backend rows were deleted in the mothball; see CV-1390/M5) |
| `LightPage.jsx` | In-app `/light` page — superseded live by `cviper-landing`'s redirect |
| `Reveal.jsx` / `useParallax.js` | Entrance + parallax motion, reduced-motion-safe |
| `BenefitIcons.jsx` / `landingStyles.js` | Shared icons and style constants |

The redesigned `LoginScreen` composes these (split out of the former
4,000-line monolith in slice 5A, PR #1395). E2E cover for these journeys is
inventoried in `docs/04-QUALITY.md` Part II.

**Open consideration (deferred):** whether the header nav gains a fifth
"New to AI?" anchor to `#landing-ai-setup` — parked in the 2026-08-31
reconciliation pending a ≥641px width re-measure, and untouched here because
the surface is not currently served.
...
|-------|----------|-----------|-----------|
| Backend Unit/Integration | pytest + pytest-xdist | 9,500+ tests, 649 files | Parallel (`-n auto --dist worksteal`) |
| Frontend Unit/Component | Vitest + React Testing Library | 5,400+ tests, 516 files | Threaded pool |
