...
- **Cron alarms could not fire, twice, for two different reasons (CV-1314, CV-1318)** — every consecutive-failure alarm in the repo was inert. `gh` had no repository context so it died with "failed to determine base repo", and the error was swallowed by `2>/dev/null || echo`, which made a broken alarm indistinguishable from a healthy system. Alarms now carry `GH_REPO`, the ATS health probe publishes its exit code, and a failed check can no longer take the same branch as a passing one. This is the second time this class has silenced the alarms (see LESSON-091); it is now auto-correction rule #71.
- **Unpatched base packages red-lined main and the preflight green-lit a SHA with no images (CV-1313)** — a Debian security fix published before the base tag was rebuilt failed the backend image build and blocked every deploy. The final Dockerfile stage now patches base packages, and the deploy preflight asserts the image tag actually exists in ACR rather than assuming a skipped build job means success.
- **A body edit could unblock a red PR (CV-1319)** — the PR-checks workflow listened for the `edited` trigger, so re-saving the description re-ran the body gates and reported green over a failing pipeline. Trigger removed.
- **CI failures now name themselves, and a hang can no longer burn the job cap (CV-1320)** — jobs that died with a two-second, no-log failure gave no clue what broke.
- **Zero salary benchmarks was a dead end (CV-1312)** — the empty state told users there was nothing to show and offered no way to add one.
- **The lint and docs-drift gates could not fail (CV-1227)** — both reported green regardless of outcome. The repo's most-repeated defect class: a guard that looks green because nothing ever asked it a question.
- **Silent-failure sweep (MD-11, MD-12, MD-13, MD-15)** — four places where a failure was reported to the user as a success. A skills-gap outage said "your CV is fine"; Settings asserted values it had failed to load; OAuth userinfo failures were swallowed; unchecked fabrication verdicts reported "clean" when they meant "unknown". All four now report the true state (rule #5).
- **The deploy auto-rollback safety net was broken (CV-1206, CV-1207, CV-1208)** — the mechanism intended to revert a bad deploy would not have run.
- **AI-output modals are isolated behind error boundaries (MD-16)** — a malformed AI response could take down the surrounding screen instead of just its own panel.
- **Salaries are no longer invented, and Australian dollars are read correctly (CV-1283)** — the estimator produced a figure where the advert stated none, and AUD amounts were parsed as sterling.
- **Waitlist states are announced to assistive technology (MD-17)**, and the tab row decides its icon fallback before paint rather than 150ms after (MD-18).
- **Terms of Service hedge restored, and the solicitor review pack built (CV-1162)**.
- **"Change in Quality settings" link now works (CV-879)** — the link on Settings → AI provider cards was a no-op (it navigated to the tab it was already on, and in Simple mode the Quality control didn't exist on the page). It now scrolls to and highlights the Quality preset in Advanced mode, and in Simple mode switches you to Advanced AI settings and lands on the highlighted control.
- **Real-backend E2E suite revived and made merge-blocking (CV-313)** — four core-funnel tests (login → analyse → score → documents) had been skipped since May blaming a backend 400; the real causes were two test bugs (a relative fetch that never reached the backend, and a missing analyse→persist step from the CV-228 contract change). Un-skipped, fixed, and the suite is now a required check on every merge (`Gate: Real-Backend E2E`).
- **Pre-push Docker smoke no longer false-blocks (#803)** — the hook now probes the Docker daemon instead of just the CLI, and honours `SKIP_DOCKER_PREPUSH=1`.
- **AI fallback content is always disclosed (CV-265)** — the Deep-Analysis panel, Portfolio Review, and generated CV/cover-letter bodies now show the standard "generated by the keyword fallback" note when AI didn't produce the content. Documents carry true per-artifact attribution: `generate_tailored_cv` previously never recorded fallback attribution on any of its six template paths, so template CVs could be labelled with whatever AI model last ran — including when validators discarded a successful AI result.

### Changed

- **CViper identifies itself honestly instead of impersonating Chrome (CV-1306)** — outbound HTTP sent a Chrome User-Agent. It now says what it is. Sites that refuse a non-browser client are handled by asking the user to paste the text instead, which is the honest failure rather than a disguised one.
- **`seen_jobs` primary key is now composite on `(url, user_id)` (CV-1199)** — one user marking a job as seen no longer affects another's view of it.
- **Generated-file freshness is enforced post-merge rather than per-branch (CV-1245)** — the stats and drift checks regenerate on main instead of failing every feature branch that happened to touch a counted file.
- **The frontend Vitest suite runs once per CI run, not twice (CV-1294)** — it was executing in two jobs, roughly doubling the frontend leg's contribution to the pipeline's cost and pushing it against a 20-minute ceiling.
- **Google Gemini catalogue: 2.5 series retired early (#894)** — Google began returning 404 for `gemini-2.5-pro`/`-flash`/`-flash-lite` on 2026-07-09, months ahead of the documented 2026-10-16 date. The three entries (and the superseded `gemini-3-flash-preview`) are removed; Google's premium and standard tiers now resolve to the flagship **Gemini 3.5 Flash**, and tier defaults will never pick a `(preview)` model while a GA model exists (new GA-before-preview resolve rule). Users with a retired model selected are auto-healed to the new default on their next AI call.
- **OpenAI catalogue: GPT-5.6 promoted to GA defaults** — Sol/Terra/Luna reached general availability 2026-07-09 and now lead the tier defaults (was GPT-4o generation).
...
- **Australia is a supported market (CV-1283, CV-1287)** — a country dimension runs through users, settings, job boards, career boards and location relevance, with a country selector on the Settings screen. Australian salary and rent data comes from official open data, and the benchmark read is paged so a large market does not block the event loop.
- **New Zealand is a supported market (CV-277)** — registries, job boards and employers, NZD salary parsing, a rent-based cost index and the statutory wage floor, plus end-to-end coverage and data provenance. Where the data has a genuine gap, the UI says so rather than filling it in.
- **Add a job from an email or advert (CV-1180)** — paste the text or attach a `.pdf`, `.docx` or `.txt` role spec and the details are extracted into a review screen where you check every field before anything is saved. If the AI is unavailable for any reason you still get the form, filled in as far as it could go. Nothing is guessed: a field the advert did not state comes back empty.
- **Cross-post duplicate detection (CV-1177, CV-1248, CV-1262)** — the same role reposted by three agencies is now recognised by a SimHash fingerprint of the description, which is the one field agencies copy verbatim. Phase A fingerprints a search batch, Phase B groups weak signals and shows a "possibly the same role" indicator you can confirm or dismiss, and Phase C persists the key so the judgement survives across searches. It re-orders and flags; it never hides a job.
- **ATS parse-fidelity harness as a hard CI gate (CV-1237, CV-1238, CV-1241, CV-1243)** — a corpus-driven measurement of how faithfully a CV survives real applicant-tracking parsers, with Greenhouse-class (NER) and Taleo-class (strict) profiles, precision scoring, per-run trend archives, and non-vacuity guards so the gate cannot pass by measuring nothing.
- **User-facing CV parse report (CV-1239)** — the harness's findings are now shown to the user as plain reasons their CV may not survive a parser, rather than staying an internal metric.
- **Two-product architecture, documented and guarded (CV-1322)** — CViper Light, the local-first desktop app, is now represented in this repository for the first time: three new diagrams (Light architecture, two-product context, job-ingestion data flow), a unified [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) covering both products, and [ADR-011](docs/adr/011-two-product-architecture.md). Twelve modules ported into Light are pinned by content hash in `docs/port-parity-manifest.yaml`, so changing one here fails the build and names what depends on it.
- **Consecutive-failure alarms on every scheduled workflow (MD-6)**.
